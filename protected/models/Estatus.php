...
array('nombre', 'length', 'max'=>45),
	...
	**/
	...
	{
		foreach ($this->$ambitos as $ambitosn )
			$ambitosn->deleteCascade();

		foreach ($this->$ejercicioFiscals as $ejercicioFiscalsn )
			$ejercicioFiscalsn->deleteCascade();

		foreach ($this->$estados as $estadosn )
			$estadosn->deleteCascade();
		...
		foreach ($this->$tipoUsuarios as $tipoUsuariosn )
			$tipoUsuariosn->deleteCascade();
